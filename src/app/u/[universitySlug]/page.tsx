...
export default async function UniversityPage({ params }: { params: { universitySlug: string } }) {
  ...
  const { data: uni } = await supabase
    .from("universities").select("*").eq("slug", params.universitySlug).eq("is_active", true).single();
  ...
  return (
    <div className="px-5 pb-10">
      ...
      <div className="mb-5">
        <h1 className="font-display text-xl font-extrabold leading-tight" style={{ color: "var(--text-primary)" }}>{uni.short_name || uni.name_en}</h1>
        {uni.short_name && <p className="text-xs mt-0.5" style={{ color: "var(--text-tertiary)" }}>{uni.name_en}</p>}
      </div>
      <UniGatedWrapper uniId={uni.id} uniName={uni.name_en} uniShortName={uni.short_name} uniSlug={uni.slug} professors={professors || []} />
    </div>
  );
}
