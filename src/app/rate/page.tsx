...
function RateInner() {
  ...
  useEffect(() => {
    ...
    fetch("/api/professor?slug=" + professorSlug).then(r => r.json()).then(d => {
      if (d.professor) {
        setProf(d.professor);
        const c = (d.professor.professor_courses || []).map((pc: any) => pc.courses).filter(Boolean);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/review", {
        ...
        body: JSON.stringify({ professor_id: prof.id, course_id: courseId, rating_quality: quality, rating_difficulty: difficulty, would_take_again: wta, attendance_mandatory: att, uses_textbook: tb, tags, comment: comment.trim() }),
      });
      setResult(await res.json());
    } catch { setResult({ error: "Submission failed." }); }
    setSubmitting(false);
  };

  const close = () => { if (step > 0 && !exitPrompt) { setExitPrompt(true); return; } router.push(professorSlug ? "/p/" + professorSlug : "/"); };
    ...
    <div className="min-h-screen flex items-center justify-center p-5">
      <div className="text-center max-w-sm">
        {result.success ? (<>
          ...
          <button onClick={() => router.push("/p/" + professorSlug)} className="w-full py-3 bg-brand-500 text-white rounded-xl font-semibold text-sm">View Profile</button>
        </>) : (<>
          <div className="text-5xl mb-4">😕</div>
          <h2 className="font-display text-xl font-bold text-red-600 mb-2">Could not Submit</h2>
          <p className="text-sm text-gray-500 mb-6">{result.error}</p>
          <button onClick={() => setResult(null)} className="py-3 px-6 bg-brand-500 text-white rounded-xl font-semibold text-sm">Try Again</button>
        </>)}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="px-5 pt-3">
        <div className="h-1 bg-brand-100 rounded-full overflow-hidden"><div className="h-full bg-gradient-to-r from-brand-500 to-brand-300 rounded-full transition-all" style={{ width: progress + "%" }} /></div>
        <p className="text-[10px] text-gray-400 mt-1.5 text-right">Step {step + 1} of 5</p>
      </div>
      {step === 0 && agg.review_count > 0 && (
        <div className="px-5 py-1 text-center"><span className="text-xs text-gray-500 bg-brand-50 px-3 py-1 rounded-full">✨ {agg.review_count} students rated</span></div>
      )}
      <div className="flex-1 px-5 py-6 overflow-auto">
        {step === 0 && (<div className="animate-fade-up">
          ...
          <div className="flex gap-2 flex-wrap mb-6">{courses.map((c: any) => (<button key={c.id} onClick={() => setCourseId(c.id)} className={"px-4 py-2 rounded-xl text-sm font-medium border transition " + (courseId === c.id ? "border-brand-500 bg-brand-50 text-brand-500" : "border-brand-100 bg-white text-gray-500")}>{c.code}</button>))}</div>
          ...
          {quality > 0 && <div className="mt-4 p-3 bg-green-50 rounded-xl border border-green-200 text-xs text-green-700 text-center animate-fade-up">✨ Nice — you are helping students this semester.</div>}
        </div>)}
        {step === 1 && (<div className="animate-fade-up">
        ...
        {step === 2 && (<div className="animate-fade-up">
        ...
        {step === 3 && (<div className="animate-fade-up">
          ...
          <div className="flex flex-wrap gap-2">{VALID_TAGS.map((tag) => { const sel = tags.includes(tag); const dis = tags.length >= 3 && !sel; return (<button key={tag} onClick={() => { if (sel) setTags(tags.filter(t => t !== tag)); else if (!dis) setTags([...tags, tag]); }} className={"px-3.5 py-2 rounded-full text-xs font-medium border transition " + (sel ? "border-brand-500 bg-brand-50 text-brand-500" : "border-brand-100 text-gray-500") + (dis ? " opacity-30 cursor-not-allowed" : " cursor-pointer")}>{sel ? "✓ " : ""}{tag}</button>); })}</div>
        </div>)}
        {step === 4 && (<div className="animate-fade-up">
          ...
          <div className="flex gap-1.5 mb-3 flex-wrap">{["What surprised you?", "Tips to get an A?", "Workload?"].map(pr => (<button key={pr} onClick={() => setComment(prev => prev || pr + " ")} className="px-2.5 py-1 rounded-lg text-[10px] bg-brand-50 border border-brand-100 text-brand-500 font-medium">💡 {pr}</button>))}</div>
          <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Share your experience..." className="w-full min-h-[120px] bg-brand-50/40 border border-brand-100 rounded-xl p-3.5 text-sm text-brand-900 outline-none focus:border-brand-500 resize-y" />
          <div className="flex justify-between mt-2"><span className="text-[10px] text-gray-400">{comment.length} chars</span>{comment.length >= 30 && <span className="text-[10px] text-green-600 font-medium">🎉 +20 points</span>}</div>
        </div>)}
      </div>
      <div className="px-5 py-4 border-t border-brand-100 bg-white">
        <div className="flex gap-2.5">
          {step > 0 && <button onClick={() => setStep(step - 1)} className="px-5 py-3.5 bg-brand-50 border border-brand-100 rounded-xl text-sm text-gray-500 font-medium">Back</button>}
          <button onClick={() => step < 4 ? setStep(step + 1) : handleSubmit()} disabled={!canProceed() || submitting} className={"flex-1 py-3.5 rounded-xl text-sm font-semibold transition " + (canProceed() ? "bg-gradient-to-r from-brand-500 to-brand-600 text-white shadow-md shadow-brand-500/25" : "bg-brand-100 text-gray-400 cursor-not-allowed")}>{submitting ? "Submitting..." : step < 4 ? "Continue" : "Submit Rating ✨"}</button>
        </div>
      ...
      {exitPrompt && (
        <div className="fixed inset-0 bg-brand-900/30 backdrop-blur-sm flex items-center justify-center p-5 z-50">
          <div className="bg-white rounded-2xl p-7 max-w-xs w-full text-center shadow-xl border border-brand-100">
            ...
            <h4 className="font-display text-lg font-bold text-brand-600 mb-1">15 seconds away!</h4>
            <p className="text-xs text-gray-500 mb-5">Your rating helps hundreds of students. Finish?</p>
            <button onClick={() => setExitPrompt(false)} className="w-full py-3 bg-gradient-to-r from-brand-500 to-brand-600 text-white rounded-xl font-semibold text-sm mb-2">Continue Rating</button>
            <button onClick={() => router.push(professorSlug ? "/p/" + professorSlug : "/")} className="w-full py-2.5 border border-brand-100 text-gray-500 rounded-xl text-xs">Save Draft and Exit</button>
          </div>
        </div>
      )}
    </div>
...
function StarPicker({ value, onChange, type }: { value: number; onChange: (v: number) => void; type: "quality" | "difficulty" }) {
  ...
  const gl = (v: number) => { if (!v) return ""; if (type === "difficulty") return v >= 4.5 ? "Very Hard" : v >= 3.5 ? "Hard" : v >= 2.5 ? "Average" : v >= 1.5 ? "Easy" : "Very Easy"; return v >= 4.5 ? "Excellent" : v >= 3.5 ? "Great" : v >= 2.5 ? "Average" : v >= 1.5 ? "Poor" : "Awful"; };
  const ac = d > 0 ? gc(d) : "#EDE8F5";
  return (
    <div>
      <div className="flex gap-1 justify-center mb-3">
        {[1,2,3,4,5].map(star => { const filled = d >= star; const half = !filled && d >= star - 0.5; return (
...
function Toggle({ label, value, onChange }: { label: string; value: boolean | null; onChange: (v: boolean) => void }) {
  return (
    <div className="mb-4">
      ...
      <div className="flex gap-2.5">{[true, false].map(v => (<button key={String(v)} onClick={() => onChange(v)} className={"flex-1 py-3 rounded-xl text-sm font-medium border transition " + (value === v ? "border-brand-500 bg-brand-50 text-brand-500" : "border-brand-100 bg-white text-gray-500")}>
        {v ? "👍 Yes" : "👎 No"}</button>))}</div>
    </div>
  );
}
